# -*- coding: utf-8 -*-
"""
Created on Thu Mar 20 15:10:26 2025
            ...
            try:
                for (x, y, w, h) in personas:
                    ...
                    if res == 1:
                        ...
                        croppedImg = img2[y:y+h,x:x+w]
                        date = datetime.now()
                        nombreArchivo = ("./capturas/" + str(date.year) + str(date.month) + str(date.day) + 
                                         str(date.hour) + str(date.minute)+str(date.second) + ".jpg")
                        cv2.imwrite(nombreArchivo, croppedImg)
                        
                cv2.namedWindow("frame", cv2.WINDOW_NORMAL)
                cv2.namedWindow("recorte", cv2.WINDOW_NORMAL)
                
                cv2.imshow("frame", frame)
                cv2.imshow("recorte", croppedImg)
